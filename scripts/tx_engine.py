...
Key responsibilities:
    ...
    - Pass-through counter-entry generation (ExampleCo/KSD reimbursements)
    - Transaction validation (account, category, currency, amount checks)
    - Claude API integration for free-text TX parsing
...
def _slugify(text: str) -> str:
    """Convert text to a URL/ID-safe slug (e.g. 'Jumbo Supermarket' -> 'jumbo-supermarket')."""
...
# Reimbursement categories loaded from config/defaults.json (pass_through.reimbursement_categories).
# Hardcoded fallback used if config file is missing — keeps pass-through balance logic intact.
_REIMBURSEMENT_CATEGORIES_FALLBACK = {
    "ExampleCo Limited": "Income:ExampleCo Reimbursement",
    "KSD": "Income:KSD Reimbursement",
}
...
def generate_pass_through_line(
    ...
    Pass-through accounts (kft, kfu, ksdu) represent money spent on behalf
    of a business entity (ExampleCo, KSD). Every expense gets a matching income
    line for reimbursement, keeping the pass-through account balance at zero.
...
def _render_smart_default_rule(rule: dict) -> str:
    """Render one entry from config/smart_defaults.json:prompt_rules as a bullet.

    Accepted shapes:
        {"description": "<full bullet text>"}                          → verbatim
        {"trigger": "X", "account": "y", "category": "Foo:Bar"}        → derived
    """
    ...
    bits = []
    if rule.get("account"):
        bits.append(f"account={rule['account']}")
    if rule.get("category"):
        bits.append(f"category={rule['category']}")
    if not bits:
        return ""
    return f"- \"{trigger}\" without account -> {', '.join(bits)}"


def build_parse_prompt(accounts: dict, categories: dict, payee_defaults: dict) -> str:
    """Build the system prompt for Claude API free-text TX parsing.

    Constructs a detailed prompt containing current accounts, categories,
    and known payees so Claude can map free-text inputs like
    '45k Jumbo cash' to structured transaction data.
    """
    # Accounts table (active only, archived accounts excluded)
    acc_lines = ["| alias | name | currency | type | pass_through_payee |", "|---|---|---|---|---|"]
    for a in sorted(accounts.values(), key=lambda x: x["alias"]):
        if a.get("status") == "archived":
            continue
        acc_lines.append(
            f"| {a['alias']} | {a['name']} | {a['currency']} | {a['type']} | {a.get('pass_through_payee', '')} |"
        )

    # Categories (active only)
    cat_lines = ["| path | type |", "|---|---|"]
    for path, c in sorted(categories.items()):
        if c.get("active", "true") == "true":
            cat_lines.append(f"| {path} | {c['type']} |")

    # Payee defaults
    payee_lines = []
    seen = set()
    for key, val in sorted(payee_defaults.items(), key=lambda x: x[1]["payee"]):
        canonical = val["payee"]
        if canonical in seen:
            continue
        seen.add(canonical)
        payee_lines.append(f"- {canonical} -> {val['category']}" + (f" (account: {val['account']})" if val["account"] else ""))

    # Smart-default rules from config/smart_defaults.json:prompt_rules.
    # Each rule shapes a single bullet that overrides Claude's default inference
    # when the trigger payee appears without an explicit account. Empty list →
    # only the generic guidance lines below remain.
    smart_rule_lines = []
    for rule in config_loader.get_smart_default("prompt_rules", []):
        bullet = _render_smart_default_rule(rule)
        if bullet:
            smart_rule_lines.append(bullet)

    return f"""You are a transaction parser for FinanceOS, a personal finance system.
Given a short free-text transaction input, extract structured data.
...
## Known Payees
{chr(10).join(payee_lines) if payee_lines else "(none)"}

## Amount conventions
- "45k" = 45000, "2.5m" = 2500000
- Currency suffix like "15 eur" means EUR. Without suffix, use account's currency.
- Default currency for accounts without explicit marker: look up the account's currency from the table above.

## Smart Defaults
{chr(10).join(smart_rule_lines) if smart_rule_lines else "- (no project-specific rules configured)"}
- When no account is specified, try to infer from context (payee defaults, currency hint).
- If you truly cannot determine the account, set account="" and add an ambiguity note.

## Transfer detection
- Keywords: "transfer", "zu", "to", "->", "nach"
- "TX transfer 500k crdb zu sav" -> type=transfer, account=crdb, transfer_to_account=sav
- For transfers: payee and category should be empty strings.

## Rules
- DO NOT generate pass-through counter-entries (the server handles those).
- DO NOT apply auto-tags (the server handles those).
- If a tag is explicitly mentioned with # (e.g. "#Vacation"), include it.
- Amounts are always positive. Direction is determined by type (expense/income/transfer).

## Output format
Respond with ONLY a JSON object, no markdown fences, no explanation:
{{"lines": [{{"date": "YYYY-MM-DD", "account": "<alias>", "type": "expense|income|transfer", "amount": <number>, "currency": "<3-letter>", "payee": "<string>", "category": "<full:path>", "note": "<string or empty>", "tags": "<semicolon-separated or empty>", "transfer_to_account": "<alias or empty>", "transfer_to_amount": <number or null>}}], "confidence": "high|medium|low", "ambiguities": ["<any questions>"]}}"""


def parse_with_claude(raw_input: str, tx_date: str) -> dict:
    """Parse free-text transaction input using the Claude API.

    Sends the raw input along with current account/category/payee context
    to Claude, which returns structured JSON. The response is then
    post-processed to add import IDs, auto-tags, and pass-through
    counter-entries.

    Args:
        raw_input: Free-text input (e.g. '45k Jumbo cash').
        tx_date: ISO date string for the transaction.

    Returns:
        Dict with 'lines' (list of TX dicts), 'confidence', 'ambiguities',
    ...
    if not api_key:
        return {"error": "ANTHROPIC_API_KEY not configured. Use manual mode.", "code": "NO_API_KEY"}

    accounts = load_accounts()
    categories = load_categories()
    payee_defaults = load_payee_defaults()
    system_prompt = build_parse_prompt(accounts, categories, payee_defaults)
    ...
    try:
        response = client.messages.create(
            ...
            messages=[{"role": "user", "content": f"Date: {tx_date}\nInput: {raw_input}"}],
        )
    ...
    for line in parsed.get("lines", []):
        # Ensure all expected fields exist (Claude may omit optional ones)
        line.setdefault("raw_note", "")
        line.setdefault("receipt_group", "")
        line.setdefault("receipt_url", "")
        line.setdefault("third_party_id", "")
        line.setdefault("note", "")
        line.setdefault("tags", "")
        line.setdefault("transfer_to_account", "")
        line.setdefault("transfer_to_amount", "")

        # Normalize amount to string for consistent downstream handling
        line["amount"] = str(line["amount"]) if line["amount"] else "0"
        if line.get("transfer_to_amount") is not None:
            line["transfer_to_amount"] = str(line["transfer_to_amount"]) if line["transfer_to_amount"] else ""

        # Auto-tags
        explicit_tags = [t for t in line.get("tags", "").split(";") if t]
        all_tags = apply_auto_tags(line.get("account", ""), line.get("payee", ""), explicit_tags)
        line["tags"] = ";".join(all_tags)
        ...
        if line.get("type") == "expense" and acc_info.get("pass_through_payee"):
...
def build_manual_lines(form_data: dict) -> dict:
    """Build transaction lines from structured form data (no Claude API needed).

    This is the manual-mode alternative to parse_with_claude. The dashboard's
    Manual tab submits pre-structured data that just needs ID generation,
    auto-tags, pass-through handling, and validation.
