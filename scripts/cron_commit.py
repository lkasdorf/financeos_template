...
def main() -> int:
    ...
    3. commit local data/ changes — Pi→PC sync direction. Done BEFORE rebase
       because git refuses `rebase` with an unstaged-changes working tree
       ("error: cannot rebase: You have unstaged changes"). With cron firing
       every 5 minutes against five separate data-mutating crons (fx, metals,
       integrity, sched, payee auto-learn), the working tree is dirty most
       of the time — committing first turns the dirty state into a real
       commit so the rebase has a clean tree to work with.
