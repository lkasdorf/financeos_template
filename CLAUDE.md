...
## Project
A self-hosted, CSV-based personal finance system. Data lives in `data/`, scripts in `scripts/`, dashboard in `dashboard/`. Bookings happen via the dashboard (manual entry) or the Claude Code terminal (TX free-text).
...
For payees / contexts where the account is unambiguous, you can omit it. Document recurring rules in this `CLAUDE.md` (this section) so Claude Code applies them on every booking — for example:

> - `Coffee Cart` defaults to account `cash`, category `Food:Coffee`
...
5. **Response style:** short, structured, no fluff. For bookings: preview → confirmation → commit message. No small talk.

---

## Where the user-specific bits live

The template ships with empty `data/accounts.csv`, `data/categories.csv`, `data/tags.csv` (just the header rows + the seed sets generated by the setup wizard). User-specific configuration — auto-tag rules, pass-through reimbursement payees — lives in `config/*.json`. Recurring smart-default rules for terminal TX bookings (e.g. "Coffee Cart defaults to cash") belong directly in this `CLAUDE.md`. As you and the user work together over time, you'll learn their accounts, payees, and category preferences from `data/accounts.csv` + `data/payees.json` + `data/categories.csv`. There is no hardcoded list in this file.
